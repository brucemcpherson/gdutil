...
import { getStorageClient } from './services.js';
...
/**
 * definition of how to address the various support modes
 */
export const specPrefixes = {
  driveProtocol: 'gd:',
  driveId: 'gd:',
  driveFolderLink: '/drive/folders/',
  driveFileLink: '/file/d/',
  driveLinkHostName: 'drive.google.com',
  driveLinkProtocol: 'https:',
  rxDrivePath: /^gd:\/\//i,
  drivePath: 'gd://'
}

/** handle wildcars */
export const hasWildCards = (text) => text.match(/\*|\?|\%/)
export const wMatch = (target) => wcmatch(target)
export const isMatch = (a, b) => wMatch(a)(b)

/**
 * 
 * @param {Chunker} chunker 
 * @returns {Promise <*[]>}an array of all the values in a chunker
 */
export const chunkerUnwind = async (chunker) => {
  // we can use arrayfromasync from node 22 - park that for now and just do this
  const files = []
...
/**
 * outs a message then exists process
 * @param {object} config message to output
 * @return void
 */
export const configExit = (config) => {
  console.error(config.error.toString(), `:${config.error.code}`)
  process.exit(1)
}

export const configError = (config, error, code) => {
  config.ok = false
  config.error = {
    toString: () => error,
...
/**
 * figure out what kind of filespec this is 
 * @param {string} fileSpec 
 * @returns {Promise <object>} config
 */
export const figureFileSpec = async (fileSpec, rp = {}) => {
  let u = null
  ...
  try {
    ...
    return configError(configModel, error.toString() + `\n` + fileSpec, 400)
  }
  // because file links have an extra /view path
  if (u.pathname.match(/\/view$/)) {
  ...
  const config = {
    ...
    paths: {
      ...
      fileBare: path.basename(u.pathname, path.extname(u.pathname)),
      folderPath: u.href.replace(specPrefixes.rxDrivePath, "")
    }
  }


  const { paths, gdrive } = config


  // format gd://folder/folder[/file]
  if (isDrivePath(config)) {
    return handleDrivePath(config)
  }

  // format gd:fileid
  else if (isDriveId(config)) {
    return handleDriveId(config)
  }

  // format https://drive.google.com/..etc  
  else if (isDriveLink(config)) {
    return handleDriveLink(config)

  } else {
    return configError(config, 'unsupported filespec' + fileSpec, 400)
  }
...
export const getDriveFolder = async ({ gdFile, name, exitOnFail }) => {
  ...
  if (folder && folder.data && !isFolderType(folder.data)) {
